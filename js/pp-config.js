...
(function () {
  ...
  // --- GRIST (à modifier uniquement ici si réimport) ---
  ...
  // Deep-link QR (routeur rapide)
  window.PP_WRAP_SRC = function (gristUrl) {
    return window.PP_SITE_BASE + "?src=" + encodeURIComponent(gristUrl || "");
  };

  // Liens header (hash routes)
  window.PP_LINKS = {
    home:   window.PP_SITE_BASE + "#home",
    portal: window.PP_SITE_BASE + "#home",
    admin:  window.PP_SITE_BASE + "#admin",
    ddfpt:  window.PP_SITE_BASE + "#ddfpt"
  };
  ...
  // Marqueur pour vérifier que c'est bien la bonne version
  window.PP_CONFIG_VERSION = "SAFE-MIN-2026-01-18";
})();
